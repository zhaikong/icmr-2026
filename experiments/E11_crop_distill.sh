...
################################################################################
# 消融实验 E11: Adapter + 图像多裁剪 + 文本裁切 + 自蒸馏
# 
# 目的：验证自蒸馏的稳定性贡献（不依赖交叉注意力）
# 
# 配置：
# ✅ HarMA Adapter
# ✅ 图像多裁剪 (2全局 + 2局部)
...
python -m main \
    ...
    --workers 4 \
    --logs ../logs/ablation/E11_crop_distill \
    --name E11_crop_distill_$(date +%Y%m%d_%H%M%S) \
    --save-frequency 10 \
    --seed 42
...
if [ $EXIT_CODE -eq 0 ]; then
    echo "✅ 实验 E11 训练完成!"
    echo "结果保存在: logs/ablation/E11_crop_distill/"
else
    echo "❌ 实验 E11 训练失败 (退出码: $EXIT_CODE)"
fi
echo "========================================"
