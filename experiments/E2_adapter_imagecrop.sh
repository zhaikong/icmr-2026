...
################################################################################
# 消融实验 E2: Adapter + 图像多裁剪
# 
# 目的：验证图像多裁剪的独立贡献（无文本裁切）
# 
# 配置：
# ✅ HarMA Adapter
# ✅ 图像多裁剪 (2全局 + 2局部)
...
python -m main \
    ...
    --precision amp \
    --workers 4 \
    --logs ../logs/ablation/E2_adapter_imagecrop \
    --name E2_adapter_imagecrop_$(date +%Y%m%d_%H%M%S) \
    --save-frequency 10 \
    --seed 42
...
if [ $EXIT_CODE -eq 0 ]; then
    echo "✅ 实验 E2 训练完成!"
    echo "结果保存在: logs/ablation/E2_adapter_imagecrop/"
else
    echo "❌ 实验 E2 训练失败 (退出码: $EXIT_CODE)"
fi
echo "========================================"
