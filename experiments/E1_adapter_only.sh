...
################################################################################
# 消融实验 E1: 只有Adapter（最小基线）
# 
# 目的：验证Adapter的基础效果，作为所有增强的基线
# 
# 配置：
# ✅ HarMA Adapter
# ❌ 图像多裁剪
...
python -m main \
    ...
    --precision amp \
    --workers 4 \
    --logs ../logs/ablation/E1_adapter_only \
    --name E1_adapter_only_$(date +%Y%m%d_%H%M%S) \
    --save-frequency 10 \
    --seed 42
...
if [ $EXIT_CODE -eq 0 ]; then
    echo "✅ 实验 E1 训练完成!"
    echo "结果保存在: logs/ablation/E1_adapter_only/"
else
    echo "❌ 实验 E1 训练失败 (退出码: $EXIT_CODE)"
fi
echo "========================================"
