...
def cosine_scheduler(base_value, final_value, warmup_length, steps):
    def _adjuster(step):
        if step < warmup_length:
            lr = base_value * (step + 1) / warmup_length
        else:
            e = step - warmup_length
            es = steps - warmup_length
            lr = final_value + 0.5 * (1 + np.cos(np.pi * e / es)) * (base_value - final_value)
        return lr
    return _adjuster