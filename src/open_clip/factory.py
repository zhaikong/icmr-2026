...
from .loss import ClipLoss, DistillClipLoss, CoCaLoss, SigLipLoss, COSMOSLoss
...
def load_state_dict(checkpoint_path: str, map_location='cpu'):
    ...
    if isinstance(checkpoint, dict) and 'state_dict' in checkpoint:
        ...
        # COSMOS checkpoint format: has 'student' key
...
def create_model(
    ...
    if pretrained and pretrained.lower() == 'openai':
        ...
        # Return all tokens and pooled output to calculate token-wise similarity
        model_cfg['vision_cfg']['output_all'] = output_all
        model_cfg['text_cfg']['output_all'] = output_all

        # Changed vision encoder pooled type
        model_cfg['vision_cfg']['pool_type'] = pool_type

        # Vision encoder attention pooling flag
        model_cfg['vision_cfg']['attentional_pool'] = attentional_pool 
        model_cfg['vision_cfg']['add_zero_attn'] = add_zero_attn 
        model_cfg['text_cfg']['attentional_pool'] = attentional_pool
        model_cfg['text_cfg']['add_zero_attn'] = add_zero_attn
        ...
        custom_text = model_cfg.pop(
            'custom_text', False) or force_custom_text or is_hf_model
        ...
        if custom_text:
            ...
            model = CLIP(**model_cfg, cast_dtype=cast_dtype, cosmos=cosmos, use_adapter=use_adapter)
        ...
        if pretrained:
            ...
            if checkpoint_path:
                logging.info(
                    f'Loading pretrained {model_name} weights ({pretrained}) from path {checkpoint_path}.')
                incompatible_keys = load_checkpoint(model, checkpoint_path)
                if incompatible_keys.missing_keys:
                    adapter_missing = [k for k in incompatible_keys.missing_keys if 'adapter' in k.lower() or 'gate' in k.lower()]
                    other_missing = [k for k in incompatible_keys.missing_keys if 'adapter' not in k.lower() and 'gate' not in k.lower()]
                    if adapter_missing:
                        logging.info(f'Missing Adapter keys (expected): {len(adapter_missing)}')
                    if other_missing:
                        logging.warning(f'⚠️ Missing Backbone keys (unexpected): {len(other_missing)}')
                        logging.warning(f'First 10: {other_missing[:10]}')
                if incompatible_keys.unexpected_keys:
                    logging.warning(f'⚠️ Unexpected keys: {len(incompatible_keys.unexpected_keys)}')
                    logging.warning(f'First 10: {incompatible_keys.unexpected_keys[:10]}')
            else:
                error_str = (
    ...
    set_model_preprocess_cfg(model, merge_preprocess_dict(
        preprocess_cfg, force_preprocess_cfg))
...
def create_model_and_transforms(
        ...
        image_interpolation: Optional[str] = None, # only effective for inference
        image_resize_mode: Optional[str] = None,
        use_imagecrop_aug: Optional[bool] = False,
        global_crops_number: Optional[int] = 2,
        local_crops_number: Optional[int] = 0, 
        crop_scale: Optional[float]=0.14,
