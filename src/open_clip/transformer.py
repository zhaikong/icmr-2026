...
class AttentionalCrossPooler(nn.Module):
    def __init__(
            self,
            d_model: int,
            context_dim: int,
        ...
        super().__init__()
        #self.query = nn.Parameter(torch.randn(n_queries, d_model))
        self.attn = nn.MultiheadAttention(d_model, n_head, kdim=context_dim, vdim=context_dim, add_zero_attn=add_zero_attn)
        self.ln_q = norm_layer(d_model)
        self.ln_k = norm_layer(context_dim)
    ...
    def forward(self, x: torch.Tensor, q: torch.Tensor):
        x = self.ln_k(x).permute(1, 0, 2)  # NLD -> LND
        #N = x.shape[1]
        q = self.ln_q(q).permute(1, 0, 2)  # (128, 1, 768) NLD -> LND
...
class ResidualAttentionBlock(nn.Module):
    def __init__(
            ...
            mmadapter: Optional[nn.Module] = None,
    ):
        super().__init__()
        ...
        # HarMA Adapter 集成
        self.mmadapter = mmadapter
        if self.mmadapter is not None:
            self.gate_adapter = nn.Parameter(torch.tensor(0.6), requires_grad=True)
    ...
    def forward(
        ...
        # HarMA Adapter 集成到 MLP 之后
        residual = x
        x = self.ln_2(x)
        x = self.mlp(x)
        if self.mmadapter is not None:
            alpha = torch.sigmoid(self.gate_adapter)
            x = alpha * self.mmadapter(x) + (1 - alpha) * x
        x = residual + self.ls_2(x)
        return x
...
class Transformer(nn.Module):
    def __init__(
            ...
            act_layer: Callable = nn.GELU,
            norm_layer: Callable = LayerNorm,
            mmadapter_list: Optional[nn.ModuleList] = None,
    ):
        super().__init__()
        self.width = width
        self.layers = layers
        ...
        self.resblocks = nn.ModuleList([
            ResidualAttentionBlock(
                ...
                mmadapter=mmadapter_list[i] if mmadapter_list is not None else None,
            )
    ...
    def init_parameters_last_transformer_layer(self):
        ...
        nn.init.normal_(last_block.attn.in_proj_weight, std=attn_std)
        nn.init.normal_(last_block.attn.out_proj.weight, std=proj_std)
        nn.init.normal_(last_block.mlp.c_fc.weight, std=fc_std)
        nn.init.normal_(last_block.mlp.c_proj.weight, std=proj_std)

        """
        # For sanity check, we freeze layers except the last one. should be commented later
        for param in self.resblocks.parameters():
            param.requires_grad = False

        for param in self.resblocks[-1].parameters():
...
class VisionTransformer(nn.Module):
    output_all: torch.jit.Final[bool]
    ...
    def __init__(
        ...
        self.transformer = Transformer(
            ...
            act_layer=act_layer,
            norm_layer=norm_layer,
            mmadapter_list=mmadapter_list,
        )

        if attentional_pool:
            self.pool_type = pool_type
            self.attn_pool = None
            self.attn_cross_pool = AttentionalCrossPooler(
                output_dim,
                output_dim,
    ...
    def init_parameters_last_transformer_layer(self):
        ...
        for param in self.transformer.resblocks[-1].parameters():
            param.requires_grad = True
        """
    ...
    def interpolate_pos_encoding(self, x, w, h):
        """
        Code from https://github.com/facebookresearch/dino/blob/main/vision_transformer.py#L174
        """
        npatch = x.shape[1] - 1
        ...
        patch_pos_embed = patch_pos_embed.permute(0, 2, 3, 1).squeeze().view(-1, dim) # (w0*h0, dim)
        return torch.cat((class_pos_embed.unsqueeze(0), patch_pos_embed), dim=0) # (w0*h0+1, dim)

    def forward(self, x: torch.Tensor, return_all=False, csa=False):
        B, nc, w, h = x.shape
        ...
        if csa: # Correlative Self-Attention mechanism in SCLIP method
        ...
        #if self.attn_cross_pool is not None:
        #    out_trans = x[:, 1:].clone()
        ...
        if self.output_all or return_all:
    ...
    def custom_attn(self, attn_layer, x, return_attn=False, with_attn=False, csa=False):
        """
        Code modified from SCLIP https://github.com/wangf3014/SCLIP/blob/main/clip/model.py#L283
        """
