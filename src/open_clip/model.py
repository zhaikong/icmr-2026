...
from .adapter import BiShareAdapter, MMadapter
...
class CLIP(nn.Module):
    ...
    def __init__(
        ...
        if use_adapter:
            # 获取层数和隐藏维度
            vision_layers = vision_cfg['layers'] if isinstance(vision_cfg, dict) else vision_cfg.layers
            text_layers = text_cfg['layers'] if isinstance(text_cfg, dict) else text_cfg.layers
            vision_width = vision_cfg['width'] if isinstance(vision_cfg, dict) else vision_cfg.width
            text_width = text_cfg['width'] if isinstance(text_cfg, dict) else text_cfg.width
            
            # 创建共享的 BiShareAdapter (用于 MMadapter 内部)
            shared_adapters = nn.ModuleList([
                BiShareAdapter(hidden_dim=128, num_heads=8) for _ in range(max(vision_layers, text_layers))
            ])
            
            # 创建图像 MMadapter
            img_adapters = nn.ModuleList([
                MMadapter(shared_adapters[i], vision_width, layer_id=i) for i in range(vision_layers)
            ])
            
            # 创建文本 MMadapter
            txt_adapters = nn.ModuleList([
                MMadapter(shared_adapters[i], text_width, layer_id=i) for i in range(text_layers)
            ])

        self.visual = _build_vision_tower(
            embed_dim, vision_cfg, quick_gelu, cast_dtype, mmadapter_list=img_adapters)

        text = _build_text_tower(embed_dim, text_cfg, quick_gelu, cast_dtype, mmadapter_list=txt_adapters)
