...
from torchvision.transforms import Normalize, Compose, RandomResizedCrop, InterpolationMode, ToTensor, Resize, \
    CenterCrop, ColorJitter, Grayscale, RandomHorizontalFlip, RandomApply, RandomGrayscale
...
def image_transform(
    ...
    if is_train:
        ...
        elif use_imagecrop_aug:
            train_transform = DataAugmentationMulticrop(mean, std, image_size, local_crops_number, global_crops_number, crop_scale)
        else:
            train_transform = [
                RandomResizedCrop(
                    image_size,
...
class DataAugmentationMulticrop(object):
    """"
    Modified from DINO augmentation code https://github.com/facebookresearch/dino/blob/main/main_dino.py#L419
    """
    def __init__(self, mean, std, image_size, local_crops_number, global_crops_number=2, crop_scale=0.14):
        local_crops_scale=(0.05, crop_scale)
        global_crops_scale=(crop_scale, 1.0)
    
        flip_and_color_jitter = Compose([
            RandomHorizontalFlip(p=0.5),
            color_jitter(brightness=0.4, contrast=0.4, saturation=0.2, hue=0.1, p=0.8),
            gray_scale(p=0.2),
        ])
        normalize = Compose([
            ToTensor(),
        ...
        self.global_transfo1 = Compose([
            RandomResizedCrop(image_size, scale=global_crops_scale, interpolation=InterpolationMode.BICUBIC),
            _convert_to_rgb,
        ...
        self.local_transfo = Compose([
            RandomResizedCrop(96, scale=local_crops_scale, interpolation=InterpolationMode.BICUBIC),
            _convert_to_rgb,
            flip_and_color_jitter,
            GaussianBlur(p=0.5),
            normalize,
    ...
    def __call__(self, image):
        assert self.global_crops_number == 2
        crops = []
